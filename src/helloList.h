...
int findUIDtoDestinationMatch(char* add1 , char* add2){
    ...
    if (strstr(destUID, myUID) == NULL) {
        printf("Substring not matched : %s not in %s", myUID, destUID);
        return 0;
    }
    else {
        printf("Substring matched : %s", strstr(destUID, myUID));
        return sizeof(myUID)-1;
    }
...
int CheckAllDestinationLabels(char* dest){
	...
	while (fNode != NULL) {
		...
		printf("\n Comparing destination : [%s] and my tier value [%s]\n",dest,temp);
