...
int _get_MACTest(struct addr_tuple *myAddr, int numTierAddr) {
	...
	while (1) {
		...
		//receive UDP sockets
//		nUDP = recvfrom(sockUDP, buffer, 2048, MSG_DONTWAIT,
...
//								dataDecapsulationUDP(portNameTemp, decapsPacket, decapsPacketSize);
							...
							if (isFWDFieldsSet() == 0) {
								...
								//printf("Forwarding on interface: %s \n",
										//fwdInterface);
								printf("\n calling dataFwd function with the parameters : %s , %s, %d \n",fwdInterface,MPLRPacketFwd,MPLRPacketSize);
